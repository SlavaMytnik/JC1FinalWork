...
/**
 * Класс StringBracket вычисляет значение
 * строкового арифметического выражения по заданному паттерну
 * (вычисляет значение выражения в скобках)
 */
public class StringBracket implements IStringOperation {
    ...
    /**
     * Конструктор класса StringBracket
     * @param calculator - экземпляр класса IStringExpressionCalculator для вычисления
     *                   значения строкового арифметического выражения внутри скобок
     */
    public StringBracket(IStringExpressionCalculator calculator) {
    ...
    public Double calculate(String expression) {
        if (expression != null && expression.length() > 0) {
            ...
            if (matcher.find()) {
                return calculator.calculate(matcher.group().substring(1, matcher.group().length() - 1));
            }
        }
